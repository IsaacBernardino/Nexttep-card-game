function Player({ name, type, deck, descart, GamePhasesManager }) {
  ...
  function RenderHand() {
    player.hand.forEach((card) => {
      if(player.type == 'SELF') {
        ev.places.playerField.playerHandCards.append(
          card.init(
            {
              player: this,
              showcard: true
  ...
  function updateHand() {
    ...
    player.hand.forEach((card) => {
      if(player.type == 'SELF') {
        ev.places.playerField.playerHandCards.append(card.init(
          {
            player: this,
            showCard: true
          }));
  ...
  function PlayCard(card, set) {
    ...
    function activeOnAmount() {
      ...
      if(player.type == 'SELF') {
        ev.places.playerField.amountZone.insertAdjacentElement('beforeend', cardAmount);
      } else {
        ev.places.enemyField.amountZone.insertAdjacentElement('beforeend', cardAmount);
      }
    ...
    function cardSpellField(isSet) {
      ...
      ev.places.playerField.effectZone.insertAdjacentElement("beforeend", cardSpellField);
